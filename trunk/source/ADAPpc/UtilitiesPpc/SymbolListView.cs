...
namespace UtilitiesPpc
{
    public partial class SymbolListView : UserControl
    {
        ...
        {
            Debug.WriteLine(string.Format("HasNextPage - _currentPage: {0} items.Count: {1}",
                _currentPage, items.Count));
            return (_currentPage + 1) * MAX_SYMBOLS_IN_PAGE < items.Count;
        }
        ...
        {
            Debug.WriteLine(string.Format("HasPreviousPage - _currentPage: {0} items.Count: {1}",
                _currentPage, items.Count));
        ...
        public void ShowPreviousPage()
        {
            if (HasPreviousPage())
            {
                SelectedIndex = MoveToPreviousPage();
                this.Invalidate();
            }
        }
